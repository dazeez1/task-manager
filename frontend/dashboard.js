...
function renderTasks(tasks) {
  if (!tasks || tasks.length === 0) {
    tasksContainer.innerHTML = '<div class="no-tasks">No tasks found. Create your first task!</div>';
    return;
  }

  tasksContainer.innerHTML = tasks.map(task => `
    <div class="task-card ${task.isCompleted ? 'completed' : ''} priority-${task.priorityLevel.toLowerCase()}">
      ...
      <div class="task-description">${escapeHtml(task.taskDescription || 'No description')}</div>
      ...
      <div class="task-meta">
        ...
        <div class="completion-toggle">
          <input type="checkbox" id="task-${task.id}" ${task.isCompleted ? 'checked' : ''} onchange="toggleTaskCompletion('${task.id}', ${task.isCompleted})">
          <label for="task-${task.id}">${task.isCompleted ? 'Completed' : 'Mark Complete'}</label>
        </div>
      </div>
    </div>
  `).join('');
...
function updateStats(tasks) {
  ...
  const highPriority = tasks.filter(task => task.priorityLevel === 'High' && !task.isCompleted).length;
